...
This use case helps simulate and analyze the network performance and customer experience of telco users using dummy data, real time streaming allowing visual insights via dashboards.
...
=== 7. Create customer_profiles table with static network dataset using spark job in Cloudera Data Engineering
In this step, you will create a Spark job that reads a static dataset of customer profiles uploaded on S3 bucket and creates an Iceberg table named `customer_profiles`. This table will be used later to enrich the raw CDR data.

==== Upload the customer dataset `customer.csv` to s3 bucket
Make sure to upload the `customer.csv` file (available in the assets folder) to your S3 bucket before proceeding.

==== Create and Run the CDE 
=== 6. Create & Enrich Iceberg Tables via spark jobs in Cloudera Data Engineering
